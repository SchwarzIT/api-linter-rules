import './pre-init.ts';
import type { SpectralDiagnostic, DiagnosticSeverity } from './spectral-types.js';
...
export async function lint(
    ...
    return results.map((r: any) => ({
        ...
        path: r.path.map((p: string | number) => typeof p === 'string' ? { tag: 's', val: p } : { tag: 'n', val: p }),
        code: typeof r.code === 'string' ? { tag: 's', val: r.code } : { tag: 'n', val: r.code },
        tags: r.tags || undefined,
        relatedInformation: r.relatedInformation?.map((ri: any) => ({
            location: {
